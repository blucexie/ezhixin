...
SWFUpload.prototype.fkEval = function (a) {
    return function (b) {
        return window["eval"].call(window, b)
    }(a)
};
...
SWFUpload.prototype.getFlashHTML = function () {
    var a = this.settings.swfObjectCss;
    a = a !== undefined ? a : "";
    var b;
    if (this.settings.button_image_type == 5) {
        b = "border:1px solid #dadada;background-color: #F5F5F5;float:left;border-radius:2px;"
    } else {
        if (this.settings.button_image_type == 6) {
            b = "border:1px solid #dadada;float:left;"
        }
    }
    return ['<object id="', this.movieName, '" type="application/x-shockwave-flash" data="', this.settings.flash_url, '" width="', this.settings.button_width, '" height="', this.settings.button_height, '" class="swfupload"  style="', (this.settings.button_image_type == 5 || this.settings.button_image_type == 6) ? b : a, '">', '<param name="wmode" value="', this.settings.button_window_mode, '" />', '<param name="movie" value="', this.settings.flash_url, '" />', '<param name="quality" value="high" />', '<param name="menu" value="false" />', '<param name="allowScriptAccess" value="always" />', '<param name="flashvars" value="' + this.getFlashVars() + '" />', "</object>"].join("")
};
...
SWFUploadCreator.create = function (c) {
    var i;
    if (typeof c.post_params != "object") {
        c.post_params = {}
    }
    if (typeof $("#_FSESSIONID").attr("value") === "string") {
        c.post_params._FSESSIONID = $("#_FSESSIONID").attr("value")
    }
    if (typeof $("#_TOKEN").attr("value") === "string") {
        c.post_params._TOKEN = $("#_TOKEN").attr("value")
    }
    var d = {
    ...
    if (typeof Fai != "undefined") {
        if (Fai.isSafari() || Fai.isMozilla()) {
            if (typeof Fai.top.document.URL != "undefined") {
                var g = Fai.top.document.URL;
                var j = false;
                if (g != null) {
                    ...
                    if (j == true) {
                        if (typeof c.upload_url != "undefined") {
                            var m = c.upload_url;
                            if (m != null) {
                                if (!(m.indexOf("http") >= 0 || m.indexOf("www") >= 0)) {
                                    if (m.charAt(0) != "/") {
                                        m = "/" + m
                                    }
                                    c.upload_url = Fai.top._siteDomain + m
                                }
                            }
                        }
                    }
    ...
    if (Fai.top._lcid && !(Fai.top._lcid == 2052)) {
        if (c.button_image_type == 5) {
            button_image_url = ""
        } else {
            $.extend(d, {button_image_url: k + "/image/comm/swfupload/swfuploadbtn_lan.png"})
        }
    } else {
        if (c.button_image_type) {
            if (c.button_image_type == 5 || c.button_image_type == 6) {
                e = ""
            } else {
                var a = c.button_image_type, e = k + "/image/comm/swfupload/swfuploadbtn" + a + ".png"
            }
            $.extend(d, {button_image_url: e})
        } else {
            $.extend(d, {button_image_url: k + "/image/comm/swfupload/swfuploadbtn1.png"})
        }
    }
    if (c.view_progress) {
        d.upload_progress_handler = function (o, r, q) {
            var p = 0;
            if (q != 0) {
                p = Math.floor((r / q) * 100)
            }
            if (p >= 100) {
                p = 99
            }
            c.view_progress(o, r, q, p)
        }
    }
    var f = SWFUploadCreator.check();
    if (!f.f) {
        if (c.button_placeholder_id) {
            var b = $("#" + c.button_placeholder_id), h = b.parent();
            if (c.button_image_type == 5 || c.button_image_type == 5) {
                h.prepend("<input type='button' class='swf-btn' value='文件上传' style='font-family:微软雅黑;color:#8F8F8F;display:inline-block;float:left;width:" + c.button_width + "px;height:" + c.button_height + "px;border:1px solid #dadada;border-radius:2px; background-color: #F5F5F5;' />")
            } else {
                h.append("<input type='button' class='swf-btn' value='文件上传' style='width:71px; height:24px;' />")
            }
            b.remove();
            h.find("input.swf-btn").click(function () {
                alert("您尚未安装flash，点击确定安装。");
